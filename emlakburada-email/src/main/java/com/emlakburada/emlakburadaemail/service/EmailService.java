...
public class EmailService {
    ...
    public void sendEmail(String emailMessage) throws MessagingException {
        Properties properties = prepareSmtpServer();
        Session session = prepareSessionWithCredentials(properties);
        int sendMessage = sendMessage(emailMessage, session);
        if (sendMessage == 0) {
            log.info("Mail başarıyla gönderildi! -> " + emailMessage);
        }
    }
    ...
    private int sendMessage(String email, Session session) throws MessagingException {
